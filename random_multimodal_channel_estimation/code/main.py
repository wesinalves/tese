...
if 'channel' in args.input:
    channel_model = modelHand.createArchitecture('channel_conv',np.prod(output_dim),input_shape, 'complete',output_dim)
...
if multimodal == 2:
    ...
    if 'coord' in args.input:
        ...
        if args.plots:
            ...
            np.savetxt(f'results/fit/loss_{model_name}.txt', (loss, val_loss))
    else:
        model = lidar_model
        model.compile(loss="mse",
                    optimizer=opt
...
#     loss = hist.history['loss']
#     val_loss = hist.history['val_loss']
#     np.savetxt('results/fit/loss.txt', (loss, val_loss))
